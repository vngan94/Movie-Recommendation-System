...
def insert_music(Movie_vn, Movie_en, link, url_img, rating, the_number_of_views,label):
    cursor.execute("""
        INSERT INTO Movie(Movie_vn, Movie_en, link, url_img, rating, the_number_of_views,label)
        VALUES(?, ?, ?, ?,?,?,?)
    """, Movie_vn, Movie_en, link, url_img, rating, the_number_of_views,label)
    cnxn.commit()
    # cnxn.close()
...
def select_polular_movie():
    cursor.execute("""
        select * from Movie order by the_number_of_views DESC
    """)
    res = [dict((cursor.description[i][0], value)
               for i, value in enumerate(row)) for row in cursor.fetchall()]
    return res
